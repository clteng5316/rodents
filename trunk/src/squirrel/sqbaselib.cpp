...
extern SQInteger _string_format(HSQUIRRELVM v);
...
static SQInteger string_rfind(HSQUIRRELVM v)
{
	...
	if(((top=sq_gettop(v))>1) && SQ_SUCCEEDED(sq_getstring(v,1,&str)) && SQ_SUCCEEDED(sq_getstring(v,2,&substr))){
		...
		SQInteger sublen = sq_getsize(v, 2);
		if (start_idx < sublen)
			start_idx = sublen;
		for (SQInteger i = len - start_idx; i >= 0; i--)
		{
			if (str[i] == substr[0] &&
				wcsncmp(&str[i+1], &substr[1], sublen - 1) == 0)
			{
				sq_pushinteger(v, i);
				return 1;
			}
...
#define SQ_TRY(expr)	do { if SQ_FAILED((expr)) goto error; } while(0)

// (this, prefix, start = 0, end = -1)
...
//	const SQChar *suffix;
	...
	SQ_TRY(sq_getstring(v, 1, &str));
	SQ_TRY(sq_getstring(v, 2, &prefix));
	len = scstrlen(prefix);
	if (top > 2)
		SQ_TRY(sq_getinteger(v, 3, &start));
	else
		start = 0;
	if (top > 3)
		SQ_TRY(sq_getinteger(v, 4, &end));
	else
		end = len;
	sq_pushbool(v, start >= 0 && start < end && end <= len &&
		wcsncmp(str, prefix + start, end - start) == 0);
	return 1;
...
static SQInteger string_endswith(HSQUIRRELVM v)
{
	...
	SQ_TRY(sq_getstring(v, 1, &str));
	SQ_TRY(sq_getstring(v, 2, &suffix));
	len = scstrlen(suffix);
	if (top > 2)
		SQ_TRY(sq_getinteger(v, 3, &start));
	else
	...
	off = scstrlen(str) - end + start;
	sq_pushbool(v, start >= 0 && start < end && end <= len && off >= 0 &&
		wcsncmp(str + off, suffix + start, end - start) == 0);
	return 1;
...
static SQInteger string_replace(HSQUIRRELVM v)
{
	...
	SQ_TRY(sq_getstring(v, 1, &str));
	len = scstrlen(str);
	SQ_TRY(sq_getstring(v, 2, &from));
	fromlen = scstrlen(from);
	...
	{
		SQ_TRY(sq_getstring(v, 3, &to));
		tolen = scstrlen(to);
	}
	if (top > 3)
		SQ_TRY(sq_getinteger(v, 4, &count));
	else
		count = INT_MAX;

	SQInteger	dstlen = len * 2;
	SQChar*		dst = sq_getscratchpad(v, dstlen * sizeof(SQChar));
	SQInteger	s = 0;
	SQInteger	d = 0;
	while (s < len && count > 0)
	{
		if (const SQChar* pos = scstrstr(str + s, from))
		{
			SQInteger	n = pos - str - s;
			if (d + n + tolen > dstlen)
			{
				dstlen = __max(dstlen * 2, d + n + tolen);
				dst = sq_getscratchpad(v, dstlen * sizeof(SQChar));
			}
			memcpy(dst + d, str + s, n * sizeof(SQChar));
			s += n + fromlen;
			d += n;
			memcpy(dst + d, to, tolen * sizeof(SQChar));
			d += tolen;
			--count;
		}
		else
	...
	if (s < len)
	{
		SQInteger	n = len - s;
		if (d + n > dstlen)
			dst = sq_getscratchpad(v, (d + n) * sizeof(SQChar));
		memcpy(dst + d, str + s, n * sizeof(SQChar));
		d += n;
	}
	sq_pushstring(v, dst, d);
	return 1;
...
static SQInteger string_repeat(HSQUIRRELVM v)
{
	const SQChar*	str;
	SQInteger		len;
	SQInteger		count;
	...
	SQChar*	dst = sq_getscratchpad(v, len * count * sizeof(SQChar));
	for (SQInteger i = 0; i < count; i++)
		memcpy(dst + len * i, str, len * sizeof(SQChar));
	sq_pushstring(v, dst, len * count);
	return 1;
}
...
SQRegFunction SQSharedState::_string_default_delegate_funcz[]={
	...
	{_SC("weakref"),obj_delegate_weakref,1, NULL },
	{_SC("startswith"), string_startswith, -2, _SC("ssnn")},
	{_SC("endswith"), string_endswith, -2, _SC("ssnn")},
	{_SC("replace"), string_replace, -3, _SC("ss.n")},
	{_SC("repeat"), string_repeat, 2, _SC("s n")},
	{0,0}
};
